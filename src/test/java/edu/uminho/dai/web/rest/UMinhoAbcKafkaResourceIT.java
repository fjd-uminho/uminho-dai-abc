package edu.uminho.dai.web.rest;
...
import edu.uminho.dai.config.KafkaProperties;
...
class UMinhoAbcKafkaResourceIT {
    ...
    void setup() {
        ...
        UMinhoAbcKafkaResource kafkaResource = new UMinhoAbcKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/u-minho-abc-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/u-minho-abc-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
